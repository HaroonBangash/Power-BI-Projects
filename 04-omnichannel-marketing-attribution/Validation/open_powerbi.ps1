<#
    Opens OmnichannelAttribution.pbip in Power BI Desktop and refreshes the model through
    the engine, so nobody has to click Refresh (PROJECT_STATE D15).
      ...
      3. waits 20 s: a refresh fired as soon as the port opens can hit the OLD schema
         while Desktop is still applying the TMDL (Phase 3 finding)
...
#>
param([switch]$Restart, [int]$TimeoutSec = 300)
$ErrorActionPreference = "Stop"
$pbip = Join-Path $PSScriptRoot "..\PowerBI\OmnichannelAttribution.pbip" | Resolve-Path
...
$tables = @("DimDate", "DimCampaign", "DimAttributionModel", "DimFunnelStage", "FactAdSpend", "FactLeadFunnel",
            "FactTouchpoint", "FactAttributionCredit", "CampaignAliasResolution", "FxRate", "ModelConfig", "SecurityUserAccess")
foreach ($t in $tables) {
    $q = $conn.CreateCommand(); $q.CommandText = "EVALUATE ROW ( ""n"", COUNTROWS ( '$t' ) )"
    $r = $q.ExecuteReader(); [void]$r.Read(); Write-Host ("  {0,-26} {1,10:n0}" -f $t, $r.GetValue(0)); $r.Close()
}
$conn.Close()
