...
DEF = ROOT / "PowerBI" / "SaaSRevenue.SemanticModel" / "definition"
DATABASE = "SaaSRevenueBI"
...
FACTS = {"FactSubscription", "FactSubscriptionMonth", "FactMRRMovement", "FactInvoice",
         "FactUsageMonthly", "FactSupportTicket", "FactAcquisition"}
...
NO_FORMAT_OK = {"Report Context", "DQ Metric Unit", "Movement Supported", "Driver Strength", "Driver Group",
                "Retention Colour", "Movement Bar Colour", "Driver Bar Colour"}
...
def main():
    ...
    check("CalcGroups", "one calculation group", len(groups) == 1, ", ".join(groups))
    ...
    # Security is by COUNTRY, applied to the customer dimension. Every fact reaches a
    # customer, so filtering the one dimension secures all seven facts - and a fact
    # that could be read around it would be a hole, so that is checked too.
    for tbl in ("DimCustomer", "SecurityUserAccess"):
        check("Security", f"RLS filters {tbl}", f"tablePermission {tbl}" in role_txt)
    check("Security", "RLS reads the user from USERPRINCIPALNAME", role_txt.count("USERPRINCIPALNAME") >= 2)
    check("Security", "RLS honours the ALL scope", '"ALL" IN UserScope' in role_txt)
    check("Security", "every fact reaches the secured dimension",
          all(any(fr[0] == f and to == ("DimCustomer", "CustomerID") for fr, to, _, _ in rels) for f in FACTS),
          ", ".join(sorted(f for f in FACTS
                           if not any(fr[0] == f and to == ("DimCustomer", "CustomerID")
                                      for fr, to, _, _ in rels))))
