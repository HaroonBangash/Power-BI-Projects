<#
    Opens EnterpriseFPA.pbip in Power BI Desktop and refreshes the model through
...
$pbip = Join-Path $PSScriptRoot "..\PowerBI\EnterpriseFPA.pbip" | Resolve-Path
...
$tables = @("DimDate", "DimEntity", "DimDepartment", "DimAccount", "DimCustomer", "DimVendor", "DimPLLine",
            "DimAgeingBucket", "DimScenario", "DimSensitivityStep", "FxRateMonthly", "FactGL", "FactFinancials",
            "FactARInvoice", "FactAPBill", "FactCashBalance", "DataQualityMetric", "ModelConfig", "SecurityUserAccess")
foreach ($t in $tables) {
...
foreach ($g in @("Plan Version", "Period View")) {
